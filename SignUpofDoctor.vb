...
Public Class SignUpofDoctor
    ...
    Private Sub ButtonSignup_Click(sender As Object, e As EventArgs) Handles ButtonSignup.Click
        ...
        Else
            ...
            Dim command As New MySqlCommand("INSERT INTO `doctor`( `f_name`, `username`, `email_id`, `password`, `contact`, `DOB`) VALUES (@name,@usn,@email,@pass,@mno,@dob)", conn.getConnection())
    ...
    Private Function usernameExist(ByVal username As String) As Boolean
        ...
        Dim command As New MySqlCommand("SELECT  `username` FROM `doctor` WHERE `username`=@usn ", conn.getConnection())
        command.Parameters.Add("@usn", MySqlDbType.VarChar).Value = username
